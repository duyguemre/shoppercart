...
class voucher_model extends Model {
	...
	public function getVoucher($code) {
		...
		$voucher_query = $this->db->query("SELECT *, vtd.name AS theme FROM " . DB_PREFIX . "voucher v LEFT JOIN " . DB_PREFIX . "voucher_theme vt ON (v.voucher_theme_id = vt.voucher_theme_id) LEFT JOIN " . DB_PREFIX . "voucher_theme_description vtd ON (vt.voucher_theme_id = vtd.voucher_theme_id) WHERE v.code = '" . $this->db->escape($code) . "' AND vtd.language_id = '" . (int)$this->setting->get('config_language_id') . "' AND v.status = '1'");
		
		if ($voucher_query->num_rows) {
			if ($voucher_query->row['order_id']) {
				$order_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order` WHERE order_id = '" . (int)$voucher_query->row['order_id'] . "' AND order_status_id = '" . (int)$this->setting->get('config_complete_status_id') . "'");
	...
	public function confirm($order_id) {
		...
		if ($order_info) {
			...
			foreach ($voucher_query->rows as $voucher) {
				...
				if (file_exists(DIR_TEMPLATE . $this->setting->get('config_template') . '/template/mail/voucher.tpl')) {
					$html = $template->fetch($this->setting->get('config_template') . '/template/mail/voucher.tpl');
				} else {
